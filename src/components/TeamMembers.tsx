...
export interface TeamMemberData {
  ...
  isAdvisory?: boolean; // Novo campo para identificar membros do comitê consultivo
}
...
const Team: React.FC<TeamProps> = ({ teamMembers, title = "The Team" }) => {
  // Separa os membros regulares e os do comitê consultivo
  const regularMembers = teamMembers.filter(member => !member.isAdvisory);
  const advisoryMembers = teamMembers.filter(member => member.isAdvisory);
  ...
  return (
    <section className="w-full bg-black text-white py-16">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {/* Renderiza primeiro os membros regulares */}
        {regularMembers.map((member, index) => (
          <TeamMember key={index} member={member} />
        ))}
        
        {/* Renderiza o título do comitê consultivo se houver membros */}
        {advisoryMembers.length > 0 && (
          <h2 className="flex text-4xl font-sans text-yellow-500 items-start mb-12 mt-16">Advisory Committee</h2>
        )}
        
        {/* Renderiza os membros do comitê consultivo */}
        {advisoryMembers.map((member, index) => (
          <TeamMember key={`advisory-${index}`} member={member} />
        ))}
      </div>
    </section>
  );
